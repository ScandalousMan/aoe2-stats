...
to offer: no entity, field or civilisation, which is exactly `gaps.py`'s `_WHOLE_BUILD_CAUSES`
case — the gap means *nothing about this build is known*, not that one field of it is missing, so
`gaps.KnowledgeGap`'s severity is computed over every field the register requires, not one, and is
`blocking` today (spec.md's edge case: a build with no snapshot is never informational).
"""
...
def snapshot_for(build: int) -> Snapshot | KnowledgeGap:
    ...
    `KnowledgeGap(cause="no-snapshot-for-build", build=build)`, never the nearest promoted snapshot
    and never the unpromoted snapshot that happens to share the same `describes_build` — promotion
    still gates resolvability (contracts/knowledge-base.md, "Promotion").
    ...
    if not matches:
        return KnowledgeGap(cause="no-snapshot-for-build", build=build)
    if len(matches) > 1:
        raise SnapshotError(
