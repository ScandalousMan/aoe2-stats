...
| composites/FavouritesList         | a                                            | packages/design-system/src/composites/FavouritesList/index.tsx:244       | hover:bg-surface-sunken → unresolved: Hover: selector "a[href=\"/players/1\"]" not resolvable against this element's own "href" | focus-visible:outline-2 focus-visible:outline-offset-ring-inset-flush focus-visible:outline-focus-ring → unresolved: FocusVisible: selector "a[href=\"/players/1\"]" not resolvable against this element's own "href" | active:bg-surface-sunken active:border-l-border-strong active:ring-2 active:ring-inset active:ring-border-strong → unresolved: Active: selector "a[href=\"/players/1\"]" not resolvable against this element's own "href" |
...
| primitives/Field                  | label                                        | packages/design-system/src/primitives/Field/index.tsx:132                | unresolved: className not fully resolved → none                                                                                 | unresolved: className not fully resolved → none                                                                                                                                                                       | unresolved: className not fully resolved → none                                                                                                                                                                           |
...
| Row | Rest               | Hover       | Focus-visible      | Press (active) | Disabled       |
| --- | ------------------ | ----------- | ------------------ | -------------- | -------------- |
| md  | 11 real call sites | Field:Hover | Field:FocusVisible | none           | Field:Disabled |
...
| `replay-availability.md`   |        2 | :157 (Badge), :158 (DownloadAction → Button — missed by the prior sweep, found in this pass)                                                                                                                                                                                                                         |
...
| `site-header.md`           |        1 | :296-298 ("`ThemeControl`'s own states … are `Menu`'s, unchanged") — no line at all in the prior tally, found in this pass                                                                                                                                                                                           |
...
**Total: 47 handoffs across 20 files with at least one** (corrected from the prior sweep's "50
across 20 files": `third-party-objection.md` −1, `player-search.md` −2, `country-flag.md` −1,
`replay-availability.md` +1 (`DownloadAction: per Button.`, :158, missed by every prior pass —
found in this remediation) — the same corrections named above, each with its own reasoning quoted
next to it rather than asserted). 9 files carry zero (`GOVERNANCE.md`, `README.md`'s own earlier sections,
...
- **F15/F16, carried to their consumers.** `ThemeControl` composes `Menu`'s `selection` variant
  ...
  **`actions`** variant instead — the generated `Menu` matrix's `actions` row now resolves a
  ...
  the "Manage" menu (`actions`, hover/focus/active not covered at all).
...
- **F19 (new, 2026-09-18 remediation of the orchestrator's REJECT on #80 — judged at the consumer's
  size, not only its variant).** `packages/design-system/specs/favourites-list.md:113-114` — "`RemoveControl`:
  `FavouriteToggle`'s own hover/focus" — and `:126`'s active claim are both false at the size this
...
remediation of the orchestrator's REJECT on #80) — recount with
`scripts/checks/state-coverage.mjs --write` plus a grep over the region between the
...
to 41 `none` / 3 `unresolved` / 113 covered. Every `unresolved` cell is a real one this pass could not
positively settle (a name only resolvable through a helper-component indirection this script does not
trace, or a candidate this pass's own guard evaluation could not narrow to one) — never a restatement
of "none remain," which was false the moment `none` stopped meaning "confirmed absent."
