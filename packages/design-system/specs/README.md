...
own block _and_ within a small window of physical lines of the hit itself (T594 M4, `reviewer`'s
fifth REJECT: block membership alone let one citation anywhere in a 30+-line block excuse every hit
in it, e.g. `AccountErasurePanel.stories.tsx` 101-135 — genuinely quote level now, not block level,
the claim this paragraph makes and `checkDeferralVocabularyCoverage`'s own source previously did not
keep), and never merely to whether its component has a row at all (the fourth REJECT's own second
...
- **N6.** `analysis-timeline.md:38-39` "a bare, unstyled … `Button` firing … has no bespoke states
  beyond `Button`'s own" — a real control in `apps/web`, outside this script's own scan of
  `packages/design-system/src` (the same boundary this package's rules draw everywhere else).
  `primary`'s own rest/hover/focus-visible/press are covered (generated `Button` matrix), so the
  handoff is true.
...
- **F15/F16, carried to their consumers (citation corrected 2026-09-19 — M2/M3: `:426` is the
  `<Menu` tag's own opening line; `triggerLabel="Manage"` sits two lines below it, past the
  `variant="actions"` line between them).** `ThemeControl` composes `Menu`'s `selection` variant
  ...
  `ProfileSummary`'s "Manage" menu (`index.tsx:426-428`, `triggerLabel="Manage"`) composes `Menu`'s
...
- **F19 (new, 2026-09-18 remediation of the orchestrator's REJECT on #80 — judged at the consumer's
  size, not only its variant; citation corrected 2026-09-19 — M2/M3: `:293` is the `<div>` wrapper
  around `FavouriteToggle`, never this line; `size="lg"` sits five lines down, on the
  `FavouriteToggle` instance itself).** `packages/design-system/specs/favourites-list.md:110-111`
...
sixth, `PrivacyNotice`'s `SectionHeading` (`index.tsx:264`), is the same `h2[tabIndex=-1]` shape as
`Dialog`'s and moved the same way. **`SiteHeader`'s skip link did not move its own coverage cell**
