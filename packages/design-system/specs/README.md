...
**Owner: T595 (Cause A closed 2026-09-21, T600 — 8i, above; Cause C's own owner is T671; Cause D
closed 2026-09-21, T596 — 8g, above).** T596 gave `ArchivalControl`'s privacy link and
`PrivacyNotice`'s contact-route link real `Link` instances (`standalone` and `inline`) with real
...
never re-dated. **Filed 2026-09-20; the one remaining `state-coverage-debt` entry (Cause C) fixes by
2026-09-27 (the
same horizon this row itself carries) — re-checked against the live tree on that date: a cell whose
own cause has changed (a sibling's coverage removed, a variant's class now read, a real
`pinned: true` story landed) gets closed for real; otherwise it is re-dated rather than left to
expire silently, the same discipline `KNOWN_UNACCOUNTED_FORCE_STATES` and
