...
- **F10 (corrected 2026-09-19; the `InlineLink` cell itself resolved the same day by this task's own
  extractor work — `findHelperCallSites`, below).** `PrivacyNotice`'s local anchors, as the generated
  Record 1 region reads them: the `Contents` entry (`index.tsx:490`) is covered on all three states,
  by `Hover`, `FocusVisible` and `Active`; the objection-form anchor (`index.tsx:740`) and the
  contact-route link (`index.tsx:797`) read `none` on every state; the `InlineLink` recipe
  (`index.tsx:242`) now also reads a confirmed `none` on every state, not `unresolved` —
  `resolveNameMatch`'s own `nth` branch no longer excludes a helper candidate outright. A
  non-`export`ed helper's every real call site in the file is enumerable
  (`findHelperCallSites`), each with its own guards; evaluated against a specific story's own scope,
  the earliest one reached stands in for the helper's own render position, the same
  one-slot-per-candidate approximation the existing line sort already makes for a
  `.map()`-rendered candidate. This is exactly the hand check this finding already carried, now said
  by the region itself rather than only in this prose: every `visualForceState` in
  `PrivacyNotice.stories.tsx` names `role: 'link'` with `nth: 0`, and the `Contents` entry sorts
  before the first `<InlineLink` call site in `index.tsx`, so all three state stories depict the
  `Contents` entry and no story depicts `InlineLink`. **`InlineLink` still owes its own three
  frames** — a new story, not the extractor, so that half of this finding stays open. The
  contact-route link additionally paints **zero** state classes of any
