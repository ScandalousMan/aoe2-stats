...
**578 stories, 3,490 baseline PNGs** under `__screenshots__/`: 3,468 story captures — every story at
...
| primitives/Menu                   | button                                       | packages/design-system/src/primitives/Menu/index.tsx:144                 | hover:bg-surface-sunken → TriggerHover                                                                                                     | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → EscapeReturnsFocusToTrigger                                                                                                                                                                                                                                                | active:bg-background active:ring-2 active:ring-border-strong → TriggerActive                                              |
...
| Row       | Rest              | Hover                            | Focus-visible                                                                | Press (active)                     | Disabled                     |
| --------- | ----------------- | -------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------- | ---------------------------- |
| actions   | 6 real call sites | Menu:TriggerHover                | none                                                                         | Menu:TriggerActive                 | Menu:ActionsWithDisabledItem |
...
**Owner: T595 (Cause A's own owner is T600, Cause C's is T671; Cause D closed 2026-09-21, T596 —
8g, above).** T596 gave `ArchivalControl`'s privacy link and `PrivacyNotice`'s contact-route link
real `Link` instances (`standalone` and `inline`) with real hover/focus-visible/active classes, and
each a clipped `Hover`/`FocusVisible`/`Active` story, so the Cause D `state-coverage-debt` entry is
deleted rather than re-dated — the same closing discipline row 8's own Method section states, "never
left behind as a passing exception." **Filed 2026-09-20; the two remaining `state-coverage-debt`
entries (Cause A, C) fix by 2026-09-27 (the
