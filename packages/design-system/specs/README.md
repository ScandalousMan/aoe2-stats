...
**586 stories, 3,538 baseline PNGs** under `__screenshots__/`: 3,516 story captures — every story at
...
ProfileSummary:BoardLongAliasFlagHoverRevealed` — labelled by the _story file's_ own basename, the
   ...
   **Record 1's last `unresolved` cell, closed (T595's extractor slice).** The decision: give
   `Menu`'s own `EscapeReturnsFocusToTrigger` a real `visualForceState`, never widen the play-focus
   branch's own policy. `buildElementCells`'s reason for marking every play-driven match `unresolved`
   is not that a play-driven focus is fake — it is that the extractor cannot tell, from source alone,
   whether a story's captured frame is the fresh page its own `play()` left, or the frame a
   _preceding_ story already captured (the ambiguity T594's amendment names). That reason does not
   bear on this one story more or less than any other play-driven story in the tree, so a policy
   change narrow enough to answer only this cell would be the special case T595 forbids; a policy
   change wide enough to answer it honestly would have to resolve the general ambiguity, which no
   change landing in this slice attempts. The honest, already-precedented answer for _this_ story is
   narrower: `CountryFlag`'s `FlagDismissedAfterEscape` is the identical shape — an overlay dismissed
   by `Escape`, focus handed back to the trigger by a script call inside the component's own close
   handler, on a page nothing has yet clicked with a _trusted_ pointer — and it is already resolved
   with a real `visualForceState` rather than left to the play-focus branch, per
   `tests/visual/stories.spec.ts`'s own measured `VisualForceState` comment (a script `.focus()` on
   such a page reliably matches `:focus-visible` in Chromium). `Menu`'s own trigger has no other
   `button`-role candidate in `Menu/index.tsx` (the footer item takes `role="menuitem"`), so the
   force needs no `name`/`nth` to disambiguate, the same shape `CountryFlag`'s own force carries.
   Record 1's cell counts move for the first time in this row's history: 44 none / 0 unresolved / 52
   covered (96 cells) — the credit lands on the cell the play function's own final assertion
   (`toHaveFocus()`) already named, so this closes a provability gap, not a coverage one. No new
   story, no rendered-content change: the added parameter drives an explicit `.focus()` at capture
   time on the same element the play function's own trusted script call already left focused, so the
   frame this force produces is the same frame the unforced capture already rendered — confirmed, not
   assumed, by `state-coverage.test.mjs`'s own new contrast case, which plants the pre-fix shape (a
   play-driven match with no `visualForceState`) and asserts it still resolves to `unresolved` even
...
| composites/SiteHeader             | a                                            | packages/design-system/src/composites/SiteHeader/index.tsx:156           | none → none                                                                                                                                | focus:not-sr-only focus:fixed focus:top-2 focus:left-4 focus:z-50 focus:rounded-control focus:border focus:border-border-strong focus:bg-surface-raised focus:px-3 focus:py-3 focus:font-sans focus:text-sm focus:font-normal focus:text-text-primary focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → SkipLinkFocusVisible | none → none                                                                                                                                            |
| composites/SiteHeader             | a                                            | packages/design-system/src/composites/SiteHeader/index.tsx:183           | hover:underline → BrandHover                                                                                                               | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → BrandFocusVisible                                                                                                                                                                                                                                                          | none → none                                                                                                                                            |
...
| primitives/Button                 | a                                            | packages/design-system/src/primitives/Button/index.tsx:189               | hover:bg-surface-sunken → AsLinkHover                                                                                                      | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → AsLinkFocusVisible                                                                                                                                                                                                                                                         | active:bg-background active:ring-2 active:ring-border-strong → AsLinkActive                                                                            |
| primitives/Button                 | button                                       | packages/design-system/src/primitives/Button/index.tsx:207               | hover:bg-surface-sunken → Hover; SecondaryHover; GhostHover; DestructiveHover; PrimaryHoverMd; GhostHoverLg                                | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → FocusVisible; SecondaryFocusVisible; GhostFocusVisible; DestructiveFocusVisible; GhostFocusVisibleLg                                                                                                                                                                       | active:bg-background active:ring-2 active:ring-border-strong → Active; SecondaryActive; DestructiveActive; GhostActive; PrimaryActiveMd; GhostActiveLg |
...
| primitives/Menu                   | button                                       | packages/design-system/src/primitives/Menu/index.tsx:144                 | hover:bg-surface-sunken → TriggerHover                                                                                                     | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → EscapeReturnsFocusToTrigger; TriggerFocusVisible                                                                                                                                                                                                                           | active:bg-background active:ring-2 active:ring-border-strong → TriggerActive                                                                           |
| primitives/Menu                   | button[role=menuitem][tabIndex=unresolved]   | packages/design-system/src/primitives/Menu/index.tsx:242                 | hover:bg-surface-sunken → FooterItemHover                                                                                                  | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → KeyboardNavigation                                                                                                                                                                                                                                                         | active:border-l-border-strong active:bg-background → FooterItemActive                                                                                  |
...
| primitives/Table                  | a                                            | packages/design-system/src/primitives/Table/index.tsx:281                | none → RowLinkHover                                                                                                                        | focus-visible:outline-ring focus-visible:outline-offset-ring focus-visible:outline-focus-ring → RowLinkFocusVisible                                                                                                                                                                                                                                                     | active:after:ring-2 active:after:ring-inset active:after:ring-border-strong → RowLinkActive                                                            |
| primitives/Text                   | (no local interactive element)               | N/A                                                                      | N/A                                                                                                                                        | N/A                                                                                                                                                                                                                                                                                                                                                                     | N/A                                                                                                                                                    |
| primitives/Tooltip                | button                                       | packages/design-system/src/primitives/Tooltip/index.tsx:259              | none → HoverRevealed; CountryFlag:FlagHoverRevealed; ProfileSummary:BoardFlagHoverRevealed; ProfileSummary:BoardLongAliasFlagHoverRevealed | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → KeyboardFocusRevealed; DismissedAfterEscape; CountryFlag:FlagKeyboardFocusRevealed; CountryFlag:FlagDismissedAfterEscape; ProfileSummary:BoardFlagKeyboardFocusRevealed                                                                                                    | pinned ? 'border-border-strong' : 'border-transparent' → Pinned                                                                                        |
...
| screens/ArchivalControl           | (no local interactive element)               | N/A                                                                      | N/A                                                                                                                                        | N/A                                                                                                                                                                                                                                                                                                                                                                     | N/A                                                                                                                                                    |
...
| screens/PrivacyNotice             | a                                            | packages/design-system/src/screens/PrivacyNotice/index.tsx:243           | hover:text-link-hover hover:decoration-2 → InlineLinkHover                                                                                 | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → InlineLinkFocusVisible                                                                                                                                                                                                                                                     | active:text-link-hover active:decoration-2 active:underline-offset-4 → InlineLinkActive                                                                |
| screens/PrivacyNotice             | h2[tabIndex=-1]                              | packages/design-system/src/screens/PrivacyNotice/index.tsx:265           | none → none                                                                                                                                | focus-visible:outline-ring focus-visible:outline-offset-ring focus-visible:outline-focus-ring → SectionHeadingFocusVisible                                                                                                                                                                                                                                              | none → none                                                                                                                                            |
| screens/PrivacyNotice             | a                                            | packages/design-system/src/screens/PrivacyNotice/index.tsx:491           | hover:text-link-hover → Hover                                                                                                              | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → FocusVisible                                                                                                                                                                                                                                                               | active:text-link-hover active:bg-surface-sunken active:ring-2 active:ring-border-strong active:rounded-control → Active                                |
| screens/PrivacyNotice             | a                                            | packages/design-system/src/screens/PrivacyNotice/index.tsx:741           | hover:bg-surface-sunken → ObjectionCallToActionHover                                                                                       | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → ObjectionCallToActionFocusVisible                                                                                                                                                                                                                                          | active:bg-background active:ring-2 active:ring-border-strong → ObjectionCallToActionActive                                                             |
...
| screens/ThirdPartyObjectionForm   | input                                        | packages/design-system/src/screens/ThirdPartyObjectionForm/index.tsx:130 | none → none                                                                                                                                | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → InputFocusVisible                                                                                                                                                                                                                                                          | none → none                                                                                                                                            |
...
| Row               | Rest                                                                                                                                                                                                                                                                                                 | Hover                                     | Focus-visible                                                   | Press (active)                              | Disabled                                                                                           |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | --------------------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| destructive\|lg   | screens/AccountErasurePanel (packages/design-system/src/screens/AccountErasurePanel/index.tsx:218)                                                                                                                                                                                                   | Dialog:Hover                              | Dialog:FocusVisible                                             | Dialog:Active                               | Dialog:PrimaryPending                                                                              |
...
| ghost\|lg         | Button:RealisticPageActions                                                                                                                                                                                                                                                                          | Button:GhostHoverLg                       | Button:GhostFocusVisibleLg                                      | Button:GhostActiveLg                        | none                                                                                               |
...
| primary\|md       | 8 real call sites                                                                                                                                                                                                                                                                                    | Button:PrimaryHoverMd                     | Callout:FocusVisible                                            | Button:PrimaryActiveMd                      | none                                                                                               |
...
| secondary\|md     | 19 real call sites                                                                                                                                                                                                                                                                                   | Button:SecondaryHover; Button:AsLinkHover | Button:SecondaryFocusVisible; Button:AsLinkFocusVisible         | Button:SecondaryActive; Button:AsLinkActive | primitives/ErrorState (packages/design-system/src/primitives/ErrorState/ErrorState.stories.tsx:57) |
| unresolved\|lg    | primitives/Dialog (packages/design-system/src/primitives/Dialog/index.tsx:127); primitives/Dialog (packages/design-system/src/primitives/Dialog/index.tsx:138)                                                                                                                                       | Dialog:Hover                              | Dialog:FocusVisible                                             | Dialog:Active                               | Dialog:PrimaryPending                                                                              |
...
| Row        | Rest              | Hover                                                            | Focus-visible                                                                         | Press (active)                                                                | Disabled |
| ---------- | ----------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | -------- |
| inline     | 8 real call sites | PrivacyNotice:ContactRouteLinkHover                              | PrivacyNotice:ContactRouteLinkFocusVisible                                            | Link:ActiveInline; PrivacyNotice:ContactRouteLinkActive                       | none     |
| standalone | 6 real call sites | Link:Hover; Footer:Hover; ArchivalControl:PrivacyNoticeLinkHover | Link:FocusVisible; Footer:FocusVisible; ArchivalControl:PrivacyNoticeLinkFocusVisible | Link:ActiveStandalone; Footer:Active; ArchivalControl:PrivacyNoticeLinkActive | none     |

#### `Menu`

| Row       | Rest              | Hover                            | Focus-visible                                                                | Press (active)                     | Disabled                     |
| --------- | ----------------- | -------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------- | ---------------------------- |
| actions   | 6 real call sites | Menu:TriggerHover                | Menu:TriggerFocusVisible                                                     | Menu:TriggerActive                 | Menu:ActionsWithDisabledItem |
| selection | 7 real call sites | Menu:Hover; Menu:FooterItemHover | Menu:FocusVisible; Menu:KeyboardNavigation; Menu:EscapeReturnsFocusToTrigger | Menu:Active; Menu:FooterItemActive | Menu:LoadingItem             |
...
| a @ packages/design-system/src/primitives/Table/index.tsx:281                | packages/design-system/src/primitives/Table/index.tsx:281 | RowLinkHover | RowLinkFocusVisible | RowLinkActive  | none     |
...
| `archival-control.md`      |        1 | `:196` "owned by `Button` and by the privacy link" (filed under **ArchivalControl**; one citation, two names — `Button` and the privacy link itself, the second true as of 2026-09-21 (T596): §5.1 makes the privacy link a real `Link` `standalone` instance, which genuinely owns those three states — closing **F11**, which recorded the second name as false while the link was still a local, classless anchor).                                                                                                                                                                                                                                                                                                                                                                                    |
...
provenance note; `Badge.stories.tsx:125-126` ("a failure belongs to whatever produced the state it names, never to the badge.") and `PrivacyNotice.stories.tsx:304-305` ("A failure belongs to the route a link leads to, never to the sentence stating the right.") are both about the `error`/
...
_pattern_, not a state handoff. `Menu.stories.tsx:329` ("already owns.") says which _story_ (this
one's own preceding sentence: which `Selection` above already depicts, `:328-329`) is about the
...
- **F2.** `ArchivalControl.stories.tsx`'s `HoverFocusActiveNotApplicable` — false for hover on the
  button half (`secondary|lg`, same elsewhere pointer as F1); the privacy-link half was F11, closed
...
- **F10 (closed 2026-09-19, T595, then fully closed 2026-09-20, T599 — the third half's own cell
  narrowed twice by T595, then closed at the root by T599's package-wide `buildStoryPropsScope`
  fix).** `PrivacyNotice`'s local anchors, as the
  ...
  the class half (it paints no state class at all, T596's own subject, unaffected by this task) and,
  as of T599, `none` for the story half too, not `unresolved`: first, the `nth` fix above correctly
  rules this element out of `InlineLinkHover`'s own range (a clean `reject`, not `ambiguous`);
  second, `resolveSelectorMatch`'s own caller excludes a candidate this story's scope confirms
  `'unreached'` before ever attempting its own attribute, the same guard-fold — and, third,
  `buildStoryPropsScope` (`scripts/checks/state-coverage.mjs`) no longer seeds a component prop with
  no destructuring default as `UNRESOLVED` merely because a story's own merged `args` do not name
  it. `ObjectionCallToActionHover`/`FocusVisible`/`Active`'s own args are `{ lastUpdated, hrefs }`,
  no `controllerContact` — evaluated against those stories' own scope, `controllerContact` now
  resolves to the real value it has at that render, `undefined` (Storybook's own `args` are the
  complete prop set, and a key absent from it is genuinely `undefined`, not unknown), so this
  anchor's guard, `controllerContact ? <a…> : …`, evaluates `'unreached'` rather than `'unresolved'`
  — a confirmed exclusion, not a decline to decide. `WithPublishedContact`
  (`PrivacyNotice.stories.tsx:39-50`) does set `controllerContact.contactRoute` to `/contact` and
  genuinely renders this anchor, but it forces no state at all, so it never enters this comparison
  either way — the gap was never "no story sets `controllerContact`," it was that the one story that
  does carries no `visualForceState`, and the three that do never set it; T599 closes it by making
  their own scope say so, rather than stay silent. `privacy-notice.md`'s handoff for the
  ...
  `Button` instance regardless of which variant its classes echo. **The contact-route half closed
  2026-09-21, T596**: `privacy-notice.md` §5.1 decided `ContactRouteLink` is the `Link` primitive at
  its default `inline` variant, and the implementing commit applied that literally — the local
  anchor this paragraph describes at `index.tsx:797` no longer exists (that line, once the import
  the same commit adds is accounted for, is inside the `Link` call itself), and its real
  hover/focus-visible/active classes are now credited on `Link`'s own `inline` row by
  `PrivacyNotice:ContactRouteLinkHover`/`FocusVisible`/`Active` (Record 3), with `controllerContact`
  supplied exactly as `WithPublishedContact` already did.
- **F10a (corrected 2026-09-19; the cell itself resolved by T595's own extractor work, `h1`-`h6`
  ...
  `index.tsx:265-267`, `tabIndex={-1}`) carries a real focus ring, and the region's own focus-visible
  ...
  second fixture.
- **F11 (pre-existing; closed 2026-09-21, T596).** `ArchivalControl`'s privacy link used to be a
  local anchor (generated Record 1, `screens/ArchivalControl`, formerly `index.tsx:137`) — zero
  state classes of any kind, confirmed directly. `archival-control.md` §5.1 decided it is the `Link`
  primitive instead, and the implementing commit applied that literally:
  `packages/design-system/src/screens/ArchivalControl/index.tsx:138`,
  `variant="standalone"` is the applied render, the generated region no longer carries a
  `screens/ArchivalControl` row at all (Record 1 tracks a local element, and this anchor is no
  longer one — `(no local interactive element)`), and the anchor's real hover/focus-visible/active
  classes are now credited on `Link`'s own `standalone` row by
  `ArchivalControl:PrivacyNoticeLinkHover`/`FocusVisible`/`Active` (Record 3).
...
- **F13 (root cause of F1-F4, F6-F7; corrected 2026-09-19; closed in part 2026-09-20, T595's own
  ...
  `md` (`Button:DestructiveHover`) and, since T600 (2026-09-21, F14's own closure below), one at
  `lg` too (`Dialog:Hover`) — `destructive/lg` was F14's own subject, a different row from the one
  this finding's own root cause names, and is closed now like the rest.
...
again</Button>` its own `FocusVisible` story renders, once candidates are narrowed to that story's
  ...
  fully-covered combination (generated Record 3's own row). **Closed 2026-09-21 (T600).**
  **Correction, 2026-09-23 (row 8's own debt-closure remediation): "Closed" here is
  `state-coverage.mjs`'s own verdict — a forcing story exists for every state this finding named,
  which remains true and is not reopened by this note. It is not a claim that every one of those
  frames is _defended_ by the comparator that runs against it. `destructive|lg`'s own hover frame
  ...
  `Dialog:FocusVisible` already targets — real coverage rather than a guess, because no Dialog story
  here ever sets `primaryAction.variant`, so every one of them (including these two new ones) renders
  the same default `destructive` this row already reads. `ghost/lg`, real only by reading before this
  task, is now real in the generated region too: `Button:GhostHoverLg`/`GhostFocusVisibleLg`/
  `GhostActiveLg`, forced directly on `Button`, at `lg`, rather than composed through
  `FavouriteToggle` — this row's own Method section, entry 8i, records why `ghost|lg` could not be
  told apart from an impossible cell before these stories existed, and the decision that closed it.
...
- **8i. Cause A's own first decision — widen the extractor, or hand-trace the twelve rows — made
  and recorded here, then executed, 2026-09-21 (T600).** `resolveClassParts`'s own `scopes.defaultScope`
  (this Method section's own "Record 1's own class half," above) resolves a `variant`/`size`-keyed
  object-literal lookup or conditional against the component's own _default_ branch only, so Guard 3
  of `classifyRecord3NoneCells` — the guard this Method section's own `anyOwnElementPainted` rule
  states — cannot tell a real, unread gap from an impossible cell for any axis row keyed on a
  non-default `variant`/`size`, for exactly the reason the deleted Cause A debt entry named. Two
  paths were open: widen `resolveClassParts` (and, with it, Guard 3) to resolve _every_ branch a
  component's own object literal carries, not only the default; or trace each of the twelve rows by
  hand against the real component source and give each its own story, leaving the extractor
  untouched. **Decision: hand-trace (chosen over widening).** Three reasons, together, not any one
  alone: (1) `Button`'s own `variantClasses`/`sizeClasses` (`index.tsx`) are a small, closed,
  directly-readable object literal — every one of the twelve rows was traceable from a single read of
  that one file, real work of minutes, not the systematic effort a widened extractor would still have
  needed _auditing_ to trust; (2) Guard 3 is the _only_ path any `'none'` cell for `Button`/`Link`/`Menu`'s
  hover, focus-visible or active state reaches in this tree today (`anyOwnElementPainted` is true for
  all three on those three states — each renders a real local element whose default branch paints a
  real class of its own — so every currently-open cell for those three states on these three
  primitives was already a Cause A candidate); `disabled` is not one of them —
  `classifyRecord3NoneCells`'s own state loop hard-codes `anyOwnElementPainted` to `false` whenever
  `state === 'disabled'`, so a `disabled` `'none'` cell can only ever decline via Guard 2
  (`siblingCovered`) or a spec answer, never Guard 3 — exactly how the Cause B permanent entry's own
  three `R3 Button disabled …` rows below are declined; widening the
  extractor for hover/focus-visible/active would not have been scoped to twelve rows, it would have
  touched every `'none'` cell those three states can ever produce on these three primitives, needing
  the "full before/after cell accounting across the whole
  generated region" this task's own brief warned about, for a payoff a direct read already delivers
  today); (3) `resolveClassParts` has been amended three times already in this same row's own history
  (T594, then T595 twice), each time under adversarial review, and each amendment's own comment blocks
  are already dense with the edge cases the first two passes missed — a fourth widening, done once
  more without that same review pressure, is exactly the shape of change this row's own Method section
  keeps warning against. Hand-tracing costs eight new stories and zero lines of extractor change;
  widening costs an audit of every `Button`/`Link`/`Menu` cell in the tree to confirm nothing moved
  silently. The twelve rows, resolved: `destructive|lg` and `primary|md`'s own hover/press were
  ...
  — removed here with no story added, the same "closing a gap re-stales its finding" discipline this
  row's own register has caught before. Record 3's own `none` count fell from 35 to 25 (the ten real
  rows above; the `Link` pair needed no cell to move, only the entry naming them removed) with zero
  lines changed in `scripts/checks/state-coverage.mjs` itself. The Cause A `state-coverage-debt` entry
  is deleted in this same commit — never left behind as a passing exception, this Method section's
  own closing discipline.

- **8j. Cause C's own first decision — what counts as a "state-conditional class" worth crediting —
  made and recorded here, then executed, 2026-09-21 (T671).** `tooltip.md` §4 active describes a
  real, painted press treatment (`border-2` reserved at rest, `border-border-strong` for as long as
  `pinned` is true) that `Tooltip/index.tsx:259` renders as a plain conditional class
  (`pinned ? 'border-border-strong' : 'border-transparent'`), never a literal `active:`-prefixed
  utility — the literal opposite of "no ring is painted," so this cell could never honestly close as
  bucket (b), and `extractPseudoClasses`'s own `active:`-prefix regex could never see it under any
  story, real or not. Two shapes were open for the class-half this task's own brief says is owed
  alongside a story: **Option A (narrow)** — scope the rule to exactly `Tooltip`'s own `pinned` shape
  (a `ConditionalExpression` whose condition is the bare identifier `pinned`, literally), widened only
  once a second real case is found; **Option B (general)** — a literal ternary against one of a fixed,
  named set of known state-shaped prop names, decided ahead of a second case ever turning up.
  **Decision: Option A (narrow), for the same three reasons 8i's own hand-trace-vs-widen decision
  already gave for a different guard in this row, restated for this one:** (1) `pinned` is the only
  real case found in this package today — a grep of every `primitives`/`composites`/`screens` source
  file for a `ConditionalExpression` whose condition reads as a persisted-boolean-shaped identifier
  (`pinned`, `pressed`, `toggled`, `active`, `selected`) other than `Tooltip`'s own finds none, so a
  fixed named set would today hold exactly one entry regardless of which option was chosen — Option
  B's only real difference is _declaring room_ for cases that do not yet exist; (2)
  `findStateConditionalClass` (`scripts/checks/state-coverage.mjs`) is new code, unreviewed by an
  adversarial pass the way `resolveClassParts` has been three times over (8i's own point 3) — a
  narrow, one-name allowlist is the smaller surface for a first review to hold correct, and a second
  real case can add its own name to `STATE_CONDITIONAL_IDENTIFIERS` in one line when it exists,
  costing nothing an early, speculative set would have saved; (3) a persisted-boolean-vs-CSS-pseudo-
  class shape is a genuine `Button`/`Menu` press-treatment departure serious enough that a name found
  by grep alone, never rendered, would risk crediting a shape this task never actually verified paints
  a real, screenshottable frame the way `Tooltip`'s own `Pinned` story does — "found, never guessed,"
  this file's own `resolveClassParts` comment states for a different resolution, the same bar this
  decision holds itself to.
  **A correction to Cause C's own debt entry, found while executing rather than assumed from it:**
  that entry's own text ("Neither exists yet") claimed the _story_ half was missing too — checked
  against the live tree rather than taken on the debt entry's own word (the same "verify, never
  assume" instruction the paragraph it replaced was itself closed under), and the claim does not
  hold: `Tooltip.stories.tsx`'s own `Pinned` story (`play: pinOpen`, a real `userEvent.click()`
  through `handleActivate`, no `visualForceState`) already existed, landed in commit `2e732202` on
  2026-09-06 (PR #67, Phase 4) — well before this debt entry was ever filed (2026-09-20). Only the extractor's
  own class-half was ever actually missing; no new story was added by this task, and
  `story-baselines.mjs`'s own count constants and README's "The baseline set, as it stands" section
  are therefore left untouched — there is no new baseline to count.
  **Mechanism, in outline:** `findStateConditionalClass` walks the _raw_, unflattened className
  expression (never `resolveClassParts`'s own output, which has already merged both ternary branches
  into one bag of strings, deliberately losing which branch belongs to which condition — that
  function's own "dual-branch conservative" rule) for a `ConditionalExpression` whose condition
  matches `STATE_CONDITIONAL_IDENTIFIERS`, and `findLocalElements` keeps the result on a new
  `el.activeStateConditional` field, separate from `el.active` (the literal-only field every other
  reader — the "ancestor of a forced descendant" pass, `noImpliedRoleReason`'s own fallback — keeps
  reading exactly as before). `buildElementCells` gates a new branch on that field: a story with no
  `visualForceState` (a forced CSS `:active` would be dishonest here — `tests/visual/stories.spec.ts`
  answers `state: 'active'` with a real, held `page.mouse.down()`, which would re-hover the trigger
  `pinOpen` deliberately moves the pointer off of, tooltip.md §10's own pinned criterion) but whose
  `play()` body drives a real `userEvent.click()` (`findPlayClickTarget`, the click-half's own
  `findPlayFocusTarget` counterpart) targeting a role this element implies is credited **directly**
  into `cells.active`, never `ambiguousReasons` the way a play-driven focus always is: `pinned` is a
  plain React boolean a click commits synchronously, not a browser heuristic a captured frame might or
  might not still show once the harness gets to it — the exact distinction that keeps `playFocus`
  permanently `unresolved` does not hold for a committed state value.
  **Before/after accounting, the whole generated region, not only the two cells this task owns:**
  record 1 moved 25 none / 65 covered → 24 none / 66 covered (90 cells); record 3 moved 25 none / 85
  covered → 24 none / 86 covered (110 cells) — one cell each, both `Tooltip`'s own `active` button
  (`index.tsx:259`), both now reading `Pinned`. `git diff` on the regenerated region confirms it:
  exactly the two `Tooltip` rows changed, nothing else in either table moved — `el.activeStateConditional`
  is non-null for no other element in the tree today (the grep in point (1) above), so the new branch
  in `buildElementCells` is reachable for no other cell regardless of how many other components' own
  `play()` functions call `userEvent.click()`. The Cause C `state-coverage-debt` entry is deleted in
  this same commit — never left behind as a passing exception, this Method section's own closing
  discipline.
...
R1 screens/PrivacyNotice hover h2@packages/design-system/src/screens/PrivacyNotice/index.tsx:265
R1 screens/PrivacyNotice active h2@packages/design-system/src/screens/PrivacyNotice/index.tsx:265
...
above; Cause D closed 2026-09-21, T596 — 8g, above). All three of this row's own `state-coverage-debt`
entries are closed; none remain.** T596 gave `ArchivalControl`'s privacy link and `PrivacyNotice`'s
...
row 8's own Method section states, "never left behind as a passing exception." T600 gave `Button`
five new stories and `Dialog`/`Menu` one and three respectively (8i, above), closing all twelve of
Cause A's own named rows (ten by new coverage, two already stale — `Link`'s own `inline` pair, closed
in passing by T596 the day before), so its own `state-coverage-debt` entry is deleted the same way,
never re-dated. T671 widened this pass's own class-half to credit a state-conditional class matched
against a component's own boolean state (8j, above), the extractor-side half Cause C's own entry was
waiting on — the `Pinned` story-side half was already real, landed in commit `2e732202` on 2026-09-06 (PR #67, Phase 4), well
before this entry was ever filed — so its own `state-coverage-debt` entry is deleted the same way,
never re-dated. **The two `state-coverage-permanent` entries (Cause B, E) carry no `fixBy` and are
never re-checked on a calendar by construction — 8g's own decision. Guard 2 itself (Cause B) and
`player-search.md`'s own spec answer (Cause E) are each unchanged since this pair was filed, and this
task's own read of both on 2026-09-21 confirms it — but the Cause B entry's own membership is not
pinned to Guard 2 alone: T596 (`8e40efb9`, earlier the same day) deleted three `screens/PrivacyNotice
… a@…index.tsx:797` rows and renumbered two `h2@…:264` rows to `:265` below, once the anchor those
three named stopped existing and the heading those two named moved, with no change to Guard 2's own
rule. The entry moves whenever the elements it names do, not only when the guard does.**
`node scripts/checks/state-coverage.mjs`
