...
'button'` candidate.
   ...
   **A `play()` function is walked separately** for the one shape that leaves real DOM focus behind
   ...
   comparison this pass actually ran. When one does share it, but `resolveNameMatch` cannot settle
   it on exactly one candidate, the cell is `'unresolved: <reason>'` instead — printed, never
   guessed, and never folded into `'none'`, which used to make an unresolved case indistinguishable
   from a real gap. A candidate with **no implied role at all** — `impliedRoleOf` returns `null` for
   a dynamic `role={…}` and for any tag `INTRINSIC_ROLE` does not carry (`tr`, `h2`, `label` among
   them) — never runs that comparison in the first place, so `'none'` would overstate an absence
   this pass never checked for; the cell is `'unresolved: <reason>'` instead, one of three: `dynamic role`
   (the attribute is present but not a literal), `ancestor of a forced descendant` (a nested
   local element, by JSX containment, was itself matched or left ambiguous for this same state —
   `Table`'s own `<tr>` around its row `<a>`, where the browser's real cascade would paint the row's
   own `hover:`/`active:` utilities too, but whether the visual harness's forced-pseudo-state
   mechanism cascades the same way is not knowable from source), or `no implied role` (neither of
   the above — the tag simply carries no role this pass can derive).
...
| primitives/Button                 | a                                            | packages/design-system/src/primitives/Button/index.tsx:189               | unresolved: className not fully resolved → none                                                                                                          | unresolved: className not fully resolved → none                                                                                                                                                                                                                                                                                                         | unresolved: className not fully resolved → none                                                                                                                                           |
| primitives/Button                 | button                                       | packages/design-system/src/primitives/Button/index.tsx:207               | unresolved: className not fully resolved → Hover                                                                                                         | unresolved: className not fully resolved → FocusVisible; SecondaryFocusVisible; GhostFocusVisible; DestructiveFocusVisible                                                                                                                                                                                                                              | unresolved: className not fully resolved → Active; SecondaryActive; DestructiveActive; GhostActive                                                                                        |
| primitives/Callout                | (no local interactive element)               | N/A                                                                      | N/A                                                                                                                                                      | N/A                                                                                                                                                                                                                                                                                                                                                     | N/A                                                                                                                                                                                       |
| primitives/Dialog                 | h2[tabIndex=-1]                              | packages/design-system/src/primitives/Dialog/index.tsx:102               | none → unresolved: no implied role                                                                                                                       | none → unresolved: no implied role                                                                                                                                                                                                                                                                                                                      | none → unresolved: no implied role                                                                                                                                                        |
...
| primitives/Menu                   | button[role=unresolved][tabIndex=unresolved] | packages/design-system/src/primitives/Menu/index.tsx:352                 | hover:bg-surface-sunken → unresolved: dynamic role                                                                                                       | focus-visible:outline-2 focus-visible:outline-offset-ring-inset-flush focus-visible:outline-focus-ring → unresolved: dynamic role                                                                                                                                                                                                                       | active:border-l-border-strong active:bg-background → unresolved: dynamic role                                                                                                             |
...
| primitives/Table                  | div[role=region][tabIndex=0]                 | packages/design-system/src/primitives/Table/index.tsx:153                | unresolved: className not fully resolved → none                                                                                                          | focus-visible:outline-ring focus-visible:outline-offset-ring focus-visible:outline-focus-ring → FocusVisible                                                                                                                                                                                                                                            | unresolved: className not fully resolved → none                                                                                                                                           |
| primitives/Table                  | tr                                           | packages/design-system/src/primitives/Table/index.tsx:237                | hover:bg-surface-sunken → unresolved: ancestor of a forced descendant (a@packages/design-system/src/primitives/Table/index.tsx:281, hover: RowLinkHover) | unresolved: className not fully resolved → unresolved: no implied role                                                                                                                                                                                                                                                                                  | active:bg-surface-sunken active:border-l-border-strong → unresolved: ancestor of a forced descendant (a@packages/design-system/src/primitives/Table/index.tsx:281, active: RowLinkActive) |
...
| screens/AccountErasurePanel       | label                                        | packages/design-system/src/screens/AccountErasurePanel/index.tsx:269     | none → unresolved: no implied role                                                                                                                       | none → unresolved: no implied role                                                                                                                                                                                                                                                                                                                      | none → unresolved: no implied role                                                                                                                                                        |
| screens/AccountErasurePanel       | input[role=checkbox]                         | packages/design-system/src/screens/AccountErasurePanel/index.tsx:270     | none → none                                                                                                                                              | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → none                                                                                                                                                                                                                                                       | none → none                                                                                                                                                                               |
...
| screens/PrivacyNotice             | a                                            | packages/design-system/src/screens/PrivacyNotice/index.tsx:242           | hover:text-link-hover hover:decoration-2 → unresolved: Hover: 4 candidates share role "link", nth 0 not orderable                                        | focus-visible:outline-2 focus-visible:outline-offset-ring focus-visible:outline-focus-ring → unresolved: FocusVisible: 4 candidates share role "link", nth 0 not orderable                                                                                                                                                                              | active:text-link-hover active:decoration-2 active:underline-offset-4 → unresolved: Active: 4 candidates share role "link", nth 0 not orderable                                            |
| screens/PrivacyNotice             | h2[tabIndex=-1]                              | packages/design-system/src/screens/PrivacyNotice/index.tsx:264           | none → unresolved: no implied role                                                                                                                       | focus-visible:outline-ring focus-visible:outline-offset-ring focus-visible:outline-focus-ring → unresolved: no implied role                                                                                                                                                                                                                             | none → unresolved: no implied role                                                                                                                                                        |
...
| destructive\|md                                            | Button:Destructive; Button:AllVariants                                                                                                                                                                                                                                                               | none                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Button:DestructiveFocusVisible                                  | Button:DestructiveActive                   | none            |
| ghost\|lg                                                  | Button:RealisticPageActions                                                                                                                                                                                                                                                                          | none                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | none                                                            | none                                       | none            |
| ghost\|md                                                  | Button:Ghost; Button:AllVariants; screens/ProfileSummary (packages/design-system/src/screens/ProfileSummary/ProfileSummary.stories.tsx:100)                                                                                                                                                          | FavouriteToggle:Hover                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Button:GhostFocusVisible; FavouriteToggle:FocusVisible          | Button:GhostActive; FavouriteToggle:Active | none            |
...
| primary\|lg                                                | 22 real call sites                                                                                                                                                                                                                                                                                   | Button:Hover                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Button:FocusVisible                                             | Button:Active                              | Button:Disabled |
...
| secondary\|md                                              | 19 real call sites                                                                                                                                                                                                                                                                                   | none                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Button:SecondaryFocusVisible                                    | Button:SecondaryActive                     | none            |
...
| (unresolved matches — no row, printed rather than dropped) | N/A                                                                                                                                                                                                                                                                                                  | unresolved: screens/ProfileSummary/ProfileSummary.stories.tsx:BoardFlagHoverRevealed — state "hover": 3 Button instances in screens/ProfileSummary match role "button" / name "Country:", none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) \| screens/ProfileSummary/ProfileSummary.stories.tsx:BoardFlagKeyboardFocusRevealed — state "focus-visible": 3 Button instances in screens/ProfileSummary match role "button" / name "Country:", none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) \| screens/ProfileSummary/ProfileSummary.stories.tsx:BoardLongAliasFlagHoverRevealed — state "hover": 3 Button instances in screens/ProfileSummary match role "button" / name "Country:", none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) | N/A                                                             | N/A                                        | N/A             |
...
| Row                                                             | Rest                                                       | Hover                       | Focus-visible               | Press (active)              | Disabled |
| --------------------------------------------------------------- | ---------------------------------------------------------- | --------------------------- | --------------------------- | --------------------------- | -------- |
| h2 @ packages/design-system/src/primitives/Dialog/index.tsx:102 | packages/design-system/src/primitives/Dialog/index.tsx:102 | unresolved: no implied role | unresolved: no implied role | unresolved: no implied role | none     |
...
| (unresolved matches — no row, printed rather than dropped) | N/A               | unresolved: composites/Footer/Footer.stories.tsx:Hover — state "hover": 2 Link instances in composites/Footer match role "link" / nth 0, none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) \| composites/Footer/Footer.stories.tsx:FocusVisible — state "focus-visible": 2 Link instances in composites/Footer match role "link" / nth 0, none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) \| composites/Footer/Footer.stories.tsx:Active — state "active": 2 Link instances in composites/Footer match role "link" / nth 0, none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) | N/A               | N/A                   | N/A      |
...
| (unresolved matches — no row, printed rather than dropped) | N/A               | unresolved: screens/ProfileSummary/ProfileSummary.stories.tsx:BoardFlagHoverRevealed — state "hover": 2 Menu instances in screens/ProfileSummary match role "button" / name "Country:", none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) \| screens/ProfileSummary/ProfileSummary.stories.tsx:BoardFlagKeyboardFocusRevealed — state "focus-visible": 2 Menu instances in screens/ProfileSummary match role "button" / name "Country:", none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) \| screens/ProfileSummary/ProfileSummary.stories.tsx:BoardLongAliasFlagHoverRevealed — state "hover": 2 Menu instances in screens/ProfileSummary match role "button" / name "Country:", none uniquely resolved (at least one candidate's own guard could not be evaluated from this story's args) | N/A                                        | N/A            | N/A                          |
...
| tr @ packages/design-system/src/primitives/Table/index.tsx:237               | packages/design-system/src/primitives/Table/index.tsx:237 | unresolved: ancestor of a forced descendant (a@packages/design-system/src/primitives/Table/index.tsx:281, hover: RowLinkHover) | unresolved: no implied role | unresolved: ancestor of a forced descendant (a@packages/design-system/src/primitives/Table/index.tsx:281, active: RowLinkActive) | none     |
...
citation found to point at the wrong line, or at a sentence that turns out not to be a deferral at
all, is corrected in place rather than carried over): **`match-history.md` gains a third citation**
(`:186-187`, the active-state deferral the prior tally's `:169, :171` missed — hover and
focus-visible only), moving that file from 2 to 3 and the total from 47 to 48.

| File                       | Handoffs | Cited lines (verbatim)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| -------------------------- | -------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GOVERNANCE.md`            |        0 | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `README.md` (§1-H4)        |        0 | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `analysis-timeline.md`     |        2 | `:39` "a bare, unstyled `Button` … has no bespoke states beyond `Button`'s own" (filed under **AnalysisTimeline**, target primitives/Button). `:288` "`Button`s (`Recompute`, \"Try requesting analysis\") follow `Button`'s own states" (same filing).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `archival-control.md`      |        2 | `:196` "owned by `Button` and by the privacy link" (filed under **ArchivalControl**; two names, `Button` and the privacy link itself — the second is F11/T596, not a real handoff since the link is local, not a `Link` instance).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `favourites-list.md`       |        1 | `:110-111` "`RemoveControl`: `FavouriteToggle`'s own hover/focus" (**corrected from the prior citation's `:113-114`, which pointed one clause too late**), elaborated at `:126` "`RemoveControl` keeps `FavouriteToggle`'s own active" — one control (`RemoveControl` → `FavouriteToggle`) named across two state clauses, counted once (filed under **FavouritesList**, target composites/FavouriteToggle — **F19**).                                                                                                                                                                                                                                                                                                                                                                             |
| `footer.md`                |        3 | `:102` "`PrivacyNoticeLink` and `ObjectionLink` only: colour moves to `link-hover`" (hover), `:106` "are now the `Link` primitive itself" (active); `:104`'s focus-visible bullet draws the standard ring directly rather than naming `Link`, so it is not counted as a third — all filed under **Footer**, target primitives/Link, `standalone`.                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `profile-summary.md`       |        6 | `:134` "switcher trigger and menu items per `Menu`" (hover, 1), `:141` "per `Button` and `Menu`" (active, 2) — `:138`'s focus-visible bullet ("standard ring on the trigger, on menu items, and on the ghost actions") describes the ring directly rather than deferring, so it is not a fourth; `:869` "the pointer over the flag opens the tooltip" (hover), `:873` "the identity bar's focus stops are now the flag …" (focus-visible), `:882` "pressing the flag pins its tooltip open" (active) — the switcher trio filed under **ProfileSummary**→Menu/Button (F15/F16-carried), the flag trio under **ProfileSummary**→Tooltip.                                                                                                                                                             |
...
| `shared-primitives.md`     |        5 | `:242` "hover / active — none; the root is not interactive. Actions inside it have their own" (filed under **N4**, Callout, no fixed owner). `:422` "The trigger button follows `Button` `secondary`'s own active recipe in full" (filed under **Menu**, target primitives/Button, `secondary` — provenance, not a gap). `:560` "hover, active and disabled all belong to the `Button`s inside it" (filed under **Dialog**, target primitives/Button — **F3**). `:615` "none unless the value is a link, in which case the standard ring applies to the link" and `:616` "none" (active) (filed under **N5**, StatValue, no fixed owner and no live instance).                                                                                                                                     |
...
| `structural-tier.md`       |        7 | `:441` "A `Panel` is never itself interactive" (filed under **N1**, Panel, no fixed owner). `:344` "none of its own. Its heading is not a control … the components inside it carry their own" (filed under **N2**, Section, no fixed owner). `:549` "Text that responds to a pointer is a `Link` (§9) or sits inside a `Button`" — two names, counted twice (filed under **N3**, Text, no fixed owner). `:552` "none of its own. `Text` is not focusable" (filed under **N3**, same). `:1036` "none of its own; the action inside it carries `Button`'s" (filed under **EmptyState**, target primitives/Button, `secondary`\|`md` — **F4**). `:1125` "none of its own; the recovery action carries `Button`'s" (filed under **ErrorState**, target primitives/Button, `secondary`\|`md` — **F4**). |
| `third-party-objection.md` |        2 | `:217` "the submit button (per `Button`)" (hover), `:230` "The button is per `Button`" (active) — filed under **ThirdPartyObjectionForm**, target primitives/Button. Corrected from the prior tally's 3: the focus-visible bullet (`:223-225`) names the same standard ring on three elements without deferring any of their _own_ states to another spec, so it is not counted here.                                                                                                                                                                                                                                                                                                                                                                                                              |
| `tooltip.md`               |        0 | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `typography-tokens.md`     |        0 | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

**Total: 48 handoffs across 20 files with at least one** (corrected in this audit from the prior
sweep's 47: `match-history.md` +1, `:186-187`, the active-state deferral `:169`/`:171` never
covered — the same shape as the file's own hover/focus-visible citations, missed because the
sentence sits four paragraphs below them, past an intervening remediation note). Previously
corrected from the prior-prior sweep's "50 across 20 files": `third-party-objection.md` −1,
`player-search.md` −2, `country-flag.md` −1, `replay-availability.md` +1 (`DownloadAction: per
Button.`, missed by every prior pass before that one). 9 files carry zero (`GOVERNANCE.md`,
...
`packages/design-system/src/{primitives,composites,screens}` for the same deferral language (`per
`Button``, `owned by`, `belong to`, `covered by`, `already covered`, `follows … states`, `carries
its own`), in a story's own comment and in a `*NotApplicable` story's rendered text alike — not
`*NotApplicable` stories only, since a comment attached to a real capturing story (`ThirdPartyObjectionForm`'s
`Hover`/`Active`) defers a sibling control's state the same way. Ten components carry at least one;
each is quoted, filed under the component whose story it is, and judged the same way as 8c.

| Component                 | Quotes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AnalysisTimeline`        | `:176` (comment, quoting the spec) and `:183` (rendered text) "follow `Button`'s own states" — target `secondary\|lg`, no own hover — **false, same as F1** (the story's own rendered text repeats the spec's claim verbatim, so it is false for the identical reason).                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `MatchDetailPanel`        | `:520-522` (comment) "already covered by `Button`'s own stories. Known false for hover" and `:532` (rendered text) "carries its own, per `Button`'s stories" — target `secondary\|lg` — **the comment already flags hover; this audit widens it**: focus-visible is also false here (`secondary\|lg`'s own fv cell is `ReplayAvailabilityList:FocusVisible; UploadControl:FocusVisible`, never `Button.stories.tsx`) and so is press (`ReplayAvailabilityList:Active`) — all three states are covered elsewhere, none by "`Button`'s own stories" as both the comment and the rendered text claim. T595's own known item (c) is this rendered text; this audit's correction is that the gap is three states wide, not one. |
| `ReplayAvailabilityList`  | `:265-266` (comment, quoting the spec) on the `Hover` story itself — not a deferral, context for why the story exists — **not counted** (this is the story that supplies `secondary\|lg`'s own coverage, not a claim of absence).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `SignInScreen`            | `:79-80` (comment, quoting the spec) and `:86` (rendered text) "carries its own, per `Button`'s stories" — target `primary\|lg`, fully covered — **true**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `ThirdPartyObjectionForm` | `:68-69` (comment on the `Hover` story) "the submit button (per `Button`)" and `:91` (comment on the `Active` story) "the button per `Button`" — target `primary\|lg`, fully covered — **true**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `AccountErasurePanel`     | `:101-111` (comment) and `:122-124` (rendered text) both claim the dialog's `destructive` action has press "already covered by `Button.stories.tsx`'s per-variant stories (…`DestructiveActive`)" — **false, same claim as F20, quoted twice**: `DestructiveActive` resolves to `destructive\|md`, never the `lg` this screen renders. Focus-visible is true in outcome but attributed to the wrong story by both quotes (`DestructiveFocusVisible`, itself `md`) — the real `lg` coverage is `Dialog:FocusVisible`, a story neither quote names (F20's own "true, by a different route than the one named").                                                                                                              |
| `ProfileSummary`          | `:493-495` (comment) "the switcher's own hover/focus/active are `Menu`'s stories (`ProfileSwitcher`)" and `:573` (rendered text, `RatingEntryHoverNotApplicable`) "the switcher trigger and its menu items carry theirs, per `Menu`" — **false for hover, same shape as F15**: `Menu`'s own trigger has no hover frame anywhere in the tree (F15), so neither quote's claim holds for hover; both are added to the F15/F16-carried finding below as this component's own story-level echo of the spec's `profile-summary.md:134` claim, not a new, separate false claim.                                                                                                                                                   |
| `ArchivalControl`         | `:78-83` (comment) already flags the rendered text below it as known-false; `:88-89` (rendered text) "the switch button and the privacy link carry their own hover, focus and active states" — **false for the privacy link (F11) and for the button's hover (F2, `secondary\|lg`)** — the comment's own self-correction is accurate; the rendered text it describes is the false artifact, left uncorrected pending T596's design decision, exactly as the comment says.                                                                                                                                                                                                                                                  |
| `DataExportPanel`         | `:125-139` (comment) and `:145-149` (rendered text) "focus-visible and press are already covered by `Button.stories.tsx`'s per-variant stories, but not its hover" — **true**: this pair was corrected by a prior remediation (row 5/H2, row 7/H4) and now states exactly what the generated `secondary\|lg` row shows (`hover: none`, `focus-visible`/`active` covered elsewhere) — the one component in this sweep whose own comment history shows the correction this audit asks for elsewhere.                                                                                                                                                                                                                         |
| `Dialog`                  | `:129-131` (comment) and `:138` (rendered text) "Hover, active and disabled all belong to the `Button`s inside it, never to the dialog itself" — **true as a blanket statement**: this specific story renders no actions at all (a malformed-call-site demonstration), so there is no variant/size this claim can be falsified against; `Dialog`'s real actions are covered per F3/F20 above, a different story's own claim.                                                                                                                                                                                                                                                                                               |

`PlayerResultRow.stories.tsx:64-65` ("Those states belong to `SearchBox`'s own `ResultsRegion`") and
`AccountErasurePanel.stories.tsx:65-71`/`ArchivalControl`'s own comment above are disabled/
loading/error/empty or hover-adjacent provenance notes, not hover/focus-visible/press deferrals, and
are outside this row's tracked axes (Record 1 tracks hover/focus-visible/active only) — read, not
counted. No component outside this list carries any deferral language in its own `*.stories.tsx`.
...
- **F7 (widened in the row-8 audit: `match-history.md` carries a third citation this finding did not
  read).** `match-history.md:169,171`, as consumed by `MatchDetailPanel` — its own
  ...
  pointer for F1, F2, F4 (at `lg`), F6 and F7 alike. **The active state carries the identical shape,
  one clause the prior sweep's `:169,171` citation missed**: `match-history.md:186-187`
  ("`DownloadAction`: per `Button`") defers press the same way, four paragraphs below the
  hover/focus-visible bullets, past an intervening remediation note — `secondary|lg`'s own press cell
  is `ReplayAvailabilityList:Active`, so the same reader-facing gap (true elsewhere, false for
  `MatchDetailPanel`'s own Storybook-only reader) holds for press too.
...
- **F15/F16, carried to their consumers.** `ThemeControl` composes `Menu`'s `selection` variant
  ...
  **`actions`** variant instead — the generated `Menu` matrix's `actions` row now resolves a
  ...
  `hover`/`focus-visible`/`active` frame confirmed** — corrected from "no … frame of any kind": the
  same matrix's own unresolved row names `BoardFlagHoverRevealed`/`BoardFlagKeyboardFocusRevealed`/
  `BoardLongAliasFlagHoverRevealed`, each targeting `role: 'button', name: 'Country:'` against
  ProfileSummary's exactly two `Menu` instances (the switcher and this "Manage" menu, `index.tsx:299`
  and `:426`) — neither carries that name literally, so `resolveNameMatch` cannot rule this menu out
  either, and the cell is genuinely `unresolved`, not a confirmed absence (T594's amendment: `'none'`
  ...
  story.
- **F17 (corrected 2026-09-19, part A).** `Table`'s row link (generated Record 1, `a @
index.tsx:281` row) carries a real ring of its own: `index.tsx:286` passes `focusRing`
  (`index.tsx:96`, `focus-visible:outline-ring focus-visible:outline-offset-ring
focus-visible:outline-focus-ring`), and the generated region's own left-hand class column shows it.
  ...
  region now reads that row's hover and active coverage as `unresolved: ancestor of a forced
descendant (a@…/Table/index.tsx:281, hover: RowLinkHover)` (and `active: RowLinkActive`) directly —
  the hand-written account this finding used to carry (a real hover/mouse-down on the row's own
  descendant link paints the `tr`'s fill too, which the script could not attribute to a story
  because nothing here names the `tr` by role) is now the generated reason string itself, not prose
  standing in for what the script could not say. `focus-visible` on the `tr` has no forced descendant
  (the row link's own focus-visible is a confirmed `none`, not a match or an ambiguity), so it reads
  `unresolved: no implied role` instead — a `tr` genuinely carries no state of its own there, only
  the reserved-but-unconfirmed hover/active band above.
...
- **F19 (new, 2026-09-18 remediation of the orchestrator's REJECT on #80 — judged at the consumer's
  size, not only its variant).** `packages/design-system/specs/favourites-list.md:110-111`
...
Dialog:FocusVisible` (real, composed through `Dialog`'s own `destructive`/`lg` confirm action —
  ...
  `destructive|md` (no `size` in its own args), never `lg`. The press half of the claim is false at
  this component's actual size; the focus-visible half is true, by a different route than the one
  named.

**Cell counts, cited from the regenerated region rather than restated by hand (2026-09-18
remediation of the orchestrator's REJECT on #80, corrected in the row-8 sweep that followed it) —
recount with `scripts/checks/state-coverage.mjs --write` plus a grep over the region between the
`state-coverage:begin`/`end` markers; do not hand-copy a number here again.** This remediation's own
count, taken once and cited as evidence rather than as a standing fact: Record 1 moved from 80 `none`
/ 0 `unresolved` / 28 covered cells to 65 `none` / 4 `unresolved` / 39 covered (plus 12 cells where
the source `className` itself could not be fully resolved, rendered `unresolved: className not fully
resolved` rather than a confirmed `none`); Record 3 moved from 49 `none` / 0 `unresolved` / 99 covered
to 41 `none` / 3 `unresolved` / 113 covered. **The row-8 sweep that followed (this pass) moved both
again**, for reasons item 4 and the two small fixes below give in full: Record 1 is now 56 `none` / 1
`unresolved` / 42 covered (9 cells now carry `unresolved: className not fully resolved`, not 12) —
`FavouritesList`'s row link resolved from `unresolved` to `covered` (item 4, -3 unresolved / +3
covered, the one `unresolved` cell now left is `Menu`'s trigger's own play-driven focus-visible,
real and unprovable, not a gap this pass closes), and three `label` rows (`SearchBox`,
`ThirdPartyObjectionForm`, `Field`, nine cells, all `none`) dropped out of Record 1 entirely because
none of the three wraps its control (the second small fix) — `Field`'s own row disappears
altogether, correctly: nothing else in that primitive's own JSX carries a local interactive
element once its label is excluded, the control itself being the caller's per `structural-tier.md`
§11. Record 3 is now 45 `none` / 3 `unresolved` / 113 covered: `Field`'s matrix gained a real `lg`
row (`Field:SizeLg`, four `none` cells) that a `findOwnStoryRenderInstances` defect had been
silently folding into the `md` row's own `rest` count — every `Field.stories.tsx` story renders
through a custom `render: (args) => <Field {...args}>…</Field>`, a spread the function's own
`resolveProp` fell straight past to the primitive's _default_ (`size: 'md'`) for, the same
shape T594's own REJECT-on-#80 fix (item 3 of that remediation, above) already fixed for a bare
JSX literal but not for this one; fixed the same way `resolveStoryAxisValues` already reads a
spread story's own `args`, guarded by
`scripts/checks/state-coverage.test.mjs`'s own fixture and a contrast case (a spread story that
never overrides the axis prop still falls back to the default, not to nothing). Every `unresolved`
cell left is a real one this pass could not positively settle (a candidate this pass's own guard or
name-match evaluation could not narrow to one, or — `Menu`'s trigger, the one cell left — a state a
`play()` leaves that this script cannot prove statically) — never a restatement of "none remain,"
which was false the moment `none` stopped meaning "confirmed absent." A helper-component
indirection one prop-pass deep (`FavouritesList`'s own `entry={entry}` into a sibling
`FavouriteRow`) is traced now too (item 4, `findHelperInvocationIterationContext`) — a plain,
same-file identifier passed straight through only; anything further (a transform, a spread, a
second indirection) still leaves the cell `unresolved` rather than guessed.
...
**Owners: T594 (this sweep — the generated region, 8c's full 29-file tally, 8c-bis's ten-component
story tally, 8d, F1-F20 and the F15/F16 consumer findings). T595 closes the enumerated set (F11 and
the contact-route half of F10 wait on a design decision, T596). Fix by 2026-09-27.**
