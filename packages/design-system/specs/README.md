...
| `Dialog`                  | `:169` "malformed call site, and hover, active and disabled all belong to the `Button`s inside it" (comment) and `:177` "Hover, active and disabled all belong to the `Button`s inside it" (rendered text) — **true as a blanket statement**: this specific story renders no actions at all (a malformed-call-site demonstration), so there is no variant/size this claim can be falsified against; `Dialog`'s real actions are covered per F3/F20 above, a different story's own claim. (Line numbers moved from `:130`/`:138` when T600 appended `Hover`/`Active` above this story in the same file.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- **8i. Cause A's own first decision — widen the extractor, or hand-trace the twelve rows — made
  ...
  needed _auditing_ to trust; (2) Guard 3 is the _only_ path any `'none'` cell for `Button`/`Link`/`Menu`
  reaches in this tree today (`anyOwnElementPainted` is true for all three — each renders a real local
  element whose default branch paints a real hover/focus-visible/active class of its own — so every
  currently-open cell for these three primitives was already a Cause A candidate; widening the
  extractor would not have been scoped to twelve rows, it would have touched every `'none'` cell these
  three primitives can ever produce, needing the "full before/after cell accounting across the whole
...
- **8j. Cause C's own first decision — what counts as a "state-conditional class" worth crediting —
  ...
  **A correction to Cause C's own debt entry, found while executing rather than assumed from it:**
  ...
  through `handleActivate`, no `visualForceState`) already existed, landed by T591/T592 on
  2026-09-12 — eight days before this debt entry was even filed (2026-09-20). Only the extractor's
...
waiting on — the `Pinned` story-side half was already real, landed by T591/T592 on 2026-09-12, well
...
never re-checked on a calendar by construction — 8g's own decision — only a change to Guard 2 itself
(Cause B) or to `player-search.md`'s own spec answer (Cause E) could ever move either, and this
task's own read of both on 2026-09-21 found neither changed.** `node scripts/checks/state-coverage.mjs`
