...
| primitives/Table                  | tr                                           | packages/design-system/src/primitives/Table/index.tsx:237                | hover:bg-surface-sunken → none                                                                 | unresolved: className not fully resolved → none                                                                                                                                   | active:bg-surface-sunken active:border-l-border-strong → none                                                             |
...
- **F15/F16, carried to their consumers.** `ThemeControl` composes `Menu`'s `selection` variant
  ...
  **`actions`** variant instead — the generated `Menu` matrix's `actions` row now resolves a
  `Disabled` frame (`Menu:ActionsWithDisabledItem`, item 4 of this remediation) but still has **no
  `hover`/`focus-visible`/`active` frame of any kind** — the variant-whose-state-stories-all-target-
  the-other-variant shape `state-coverage.test.mjs`'s sixth fixture plants directly.
  `profile-summary.md:134,141` do not distinguish the switcher (`selection`, partially covered) from
  the "Manage" menu (`actions`, hover/focus/active not covered at all). **Carried further, into the
  ...
  story.
- **F17 (corrected 2026-09-18).** `Table`'s row link (generated Record 3, `Table` matrix, `a @
index.tsx:281` row) — `focus-visible: none`, a real gap: the row link carries no ring of its own
  and no story forces its focus-visible state directly. The scroll region
  (`div[role=region] @ index.tsx:153`) does have a real `FocusVisible` story, now resolved in the
  generated region (its name, "Recent matches", comes from a literal `caption` prop the `FocusVisible`
  story's own `render` passes, not from `args` — the prior region showed this cell `none` too, a
  false gap this remediation's name resolution now corrects). The row's reserve-then-paint hover/
  press classes live on the `<tr>` (`index.tsx:237`) — its own generated row shows `hover`/`active`
  class text present but coverage `none`, because no `visualForceState` in this tree targets a `tr`
  by role. That `none` is not "the `tr` is not a focus/interaction target": a real hover or
  mouse-down on the row's own descendant link paints the `tr`'s fill too (the `<a>` is contained in
  the `<tr>`'s box, so `RowLinkHover`/`RowLinkActive` — which do force the link's own state — paint
  it as a side effect), which the script cannot attribute to a story because nothing here names the
  `tr` by role. Read as: covered in practice by `RowLinkHover`/`RowLinkActive`, unresolved by the
  script's own accounting, never a confirmed `none`.
