...
| 2026-09-12 | `ring-offset-inset`                                  | border                  | admitted | Applying `contracts/token-families.md` §5's test to T586's shipped `-outline-offset-4` (`Button` primary, `DataExportPanel`'s download link), found by pre-merge review of PR #74 as a value with no token behind it. **1. Decision**: an inward focus-ring offset whose magnitude leaves a fill-coloured band between the ring and a ringed control's edge, for a control that rings in `accent-contrast` against its own `accent` fill (color-tokens.md §5's DS-10 mechanism) rather than in `focus-ring` against a page surface. **2. Call sites**: `Button`'s `primary` variant and `DataExportPanel`'s download link — both already shipped the bare `-outline-offset-4` literal before this admission, well past the two-site floor. **3. Synonym search**: `ring-offset` (2px) is not a synonym — §5's geometric condition requires the inward magnitude to _strictly exceed_ the ring's own 2px width, so 2px cannot serve the same purpose; no other `border.json` member is a length, and no other family holds one. No synonym exists. **4. Utility class**: none existed — the family's `@utility` blocks named `border-hairline`/`outline-ring`/`outline-offset-ring` only. Extended `packages/design-system/tokens/build-tokens.mjs`'s `borderUtilityBlocks` to also emit `outline-offset-ring-inset`, mapped onto the same `outline-offset` property `outline-offset-ring` already uses; the JSON value (`-4px`) carries its own sign, so the utility needs no separate inward/outward branch. Consumed as `outline-offset-ring-inset` (`contracts/token-families.md` §2's Widths row). **5.** Not a colour role, so step 5 is inapplicable. **Rendering is unchanged**: `outline-offset: -4px` is what both call sites already computed to; only the class name changed, from an arbitrary numeric literal invisible to `scripts/checks/token-scale.mjs`'s bracket/px-rem-ms scan to a named, admitted token. `border.json`'s `$comment` records the same reasoning beside the value. |
