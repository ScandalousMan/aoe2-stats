...
- The ring must be **inward** — negative-offset, the same direction `MatchRow`, `PlayerResultRow`,
  ...
  fill. `Button` `primary` and `DataExportPanel`'s download link ring at `-outline-offset-4` on
  `outline-2` to satisfy this — a different offset from the four row components named above, which
  clear the page surfaces they ring on and so are unaffected by this condition and keep
  `-outline-offset-2`; the shape ("inward") is the same, the magnitude is not.
...
| File                                                           | From                                                                   | To                                                                 |
| -------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------ |
| `packages/design-system/src/primitives/Button/index.tsx`       | `primary` shares the shared `outline-focus-ring outline-offset-2` ring | `primary` overrides to `outline-accent-contrast -outline-offset-4` |
| `packages/design-system/src/screens/DataExportPanel/index.tsx` | `focus-visible:outline-focus-ring` + `outline-offset-2`                | `focus-visible:outline-accent-contrast` + `-outline-offset-4`      |
...
this section exists to prevent. Corrected to `-outline-offset-4`, stated above and guarded by
`tokens/accent-contrast-ring.test.mjs`.
