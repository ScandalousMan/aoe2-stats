...
| 10  | Reference recordings of public matches committed to this repository (006 T608) | any AoE2 player who took part in a match whose recording is committed under `tests/fixtures/replays/` — today two archives, a ranked 1v1 (2 participants) and a ranked 2v2 (4 participants); the participants are strangers to this service, not its users, and never asked for their game to be committed | the `.aoe2record` recording of each such match, byte-for-byte inside its original publisher zip — every participant's in-game name, profile identifier, actions and in-game chat text. The chat is inside the committed bytes and cannot be removed without destroying the fixture. No other column of this table's data is added by this activity | legitimate interest (Art. 6-1-f) — see the balancing test below. **No existing basis wording fits exactly and this row is flagged for the controller's review**: the closest is the public-recording basis of constitution IX (activity 9), but that basis is justified by principle IV recomputability and by safeguards (never served, read only by the analyzer) that a public repository does not have. The interest here is test evidence, not analysis publication | aoe.ms, the publisher's own download endpoint, fetched once by the maintainer; the matches themselves are public in the game's public match history | indefinite, and not bounded by this service: a committed file lives in the git history of a public repository, is not deleted by any erasure, export or objection route in `apps/api`, has no expiry, and would stay in any fork or clone already taken even if later removed from history | anyone who can read the repository, and every fork or clone of it — the one activity in this table whose recipient is the public |
...
## Balancing test for activity 10

Added by `specs/006-replay-analysis-foundations` T608, in the same change that commits the recordings
(T601). This activity is **not** activity 9 and must not be read as an extension of it: activity 9's
safeguards are that a recording is never served to anyone and is read only by `apps/analyzer`, and a
file in a public repository meets neither. It covers every committed recording, the first fixture
included, which had no entry before.

- **Interest pursued**: parser and analysis tests that read a real recording rather than a fabricated
  one, so that what this project claims a replay contains can be re-measured by anyone from bytes the
  source actually produced (constitution IV). The source purges a recording after about 31 days
  (`docs/data-sources.md` §2), so a measurement that names a recording not kept somewhere cannot be
  repeated.
- **Necessity**: a recording cannot be pseudonymised, trimmed of chat or stripped of names without
  modifying it, and modifying it destroys the very bytes the parser tests exist to read; a fabricated
  or scrubbed substitute would make those tests pass without exercising the real format. That
  establishes that this exact artifact is needed for the stated purpose. It does not establish that
  committing it to a public repository is the only way to hold it: a private store the tests fetch
  from would be less exposed, at the cost of tests any contributor or CI run cannot reproduce. That
  trade is a product decision, recorded here as one.
- **Impact on the data subject**: real, and higher than activity 9's, stated plainly rather than
  minimised. Participants' in-game names and profile identifiers, and the text of their in-game chat,
  sit in the history of a public repository. No erasure or export endpoint reaches them; no expiry
  bounds them; and removing the files from history later would not reach any fork or clone already
  taken. A subject who objects cannot be given deletion by this service. What moderates the impact:
  the names, identifiers and match are already public at the source, on the same footing activities 2
  and 4 treat them under; the archives are unmodified publisher output, not this service's derivative
  of them; and the recordings are used only by tests, never displayed, indexed or served by the
  running service. Chat text is the most sensitive element and it is committed nonetheless, because it
  cannot be separated from the bytes.
- **Safeguards**: few and stated as few. Only recordings of public matches, only what the tests need
  (two archives today, of 2 and 4 participants); the fixtures README records provenance and
  checksums; no player name is repeated in this register, the specs or the README beyond what the
  bytes themselves carry; nothing in the running service reads these files. An objection from a
  participant is handled by the controller directly and can, at most, remove the file from the current
  tree and rewrite history — it cannot reach a copy already taken, and this section does not claim
  otherwise.
- **Outcome**: the interest is not overridden, narrowly, and this is the weakest outcome in this
  register. The data is already public at its source, the purpose cannot be met with a substitute, and
  the recordings are inert in the running service. The exposure is not mitigated so much as accepted:
  it is permanent in effect, and the accepting is the controller's to confirm. **This is the same class
  of exposure the last item under "Open items before public launch" weighs for a committed provider
  fixture, and this section does not close that item.** It stays open, unticked, and now has a second
  member: the decision whether committed real-player data needs scrubbing from git history before
  public launch covers these recordings as well as that fixture.
