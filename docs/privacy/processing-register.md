...
| 7   | Player-name search, including a player who appears in nobody's match          | any AoE2 player whose name is searched — not only the opponents and teammates a consenting user has played, which is the class activity 4's necessity argument is built around and does not cover here | Relic profile_id, alias, country, clan, games played — the fields `PlayerSearchResult` carries once FR-004b strips the source's account-linking assertions (`steamId`, `shared`, `sharedHistory`, `hidden`) at the boundary; the degraded fallback (FR-004d) returns the same shape read from `aoe_profiles`, already covered by activity 4 and adds no category of its own | legitimate interest (Art. 6-1-f) — see the balancing test below                                                                                                                 | `data.aoe2companion.com`'s public search endpoint (`GET /api/profiles?search=`, `docs/data-sources.md` §3); the degraded fallback is a read of `aoe_profiles`, sourced under activity 4 | `profile_search_cache`: TTL-bounded and pruned opportunistically on write, never on a schedule (FR-044) — see `PLAYER_SEARCH_CACHE_TTL_SECONDS`, `.env.example`; no row survives past that window regardless of who searched or how often                                | none       |
...
- **Impact on the data subject**: higher than activity 4's, and stating the difference plainly is the
  ...
  finding rests on "capture is limited to matches the consenting user played in" — a constraint this
...
- **Safeguards**: FR-004b strips every account-linking field the source returns before anything
  ...
  "who is this player" beyond what the source's own public endpoint already would; the cache is
  TTL-bounded and self-pruning, per the retention column above.
...
cached search result does not describe an archived match. The only thing that ever removes a subject's
row is the TTL. That is a genuine limit on this activity's rights coverage, not a gap this paragraph is
smoothing over: nothing between a search being made and the TTL expiring lets a subject who notices
themselves cached ask for it to be removed sooner.
