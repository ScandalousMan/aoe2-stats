...
- [ ] T316 [US1] Implement the local fallback over `aoe_profiles` in the same module, and remove T314's T316-reasoned markers. It introduces no source and no request — `aoe_profiles` already holds every participant of every match this service has seen, populated by 001's discovery
