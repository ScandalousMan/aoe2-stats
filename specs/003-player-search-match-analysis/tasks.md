...
- [x] T316 [US1] Implement the local fallback over `aoe_profiles` in the same module, and remove T314's T316-reasoned markers. It introduces no source and no request — `aoe_profiles` already holds every participant of every match this service has seen, populated by 001's discovery. **Names FR-004a** (added 2026-08-29): case-insensitive, partial-string matching ordered most-played-first is this task's work on the fallback path and the source's own behaviour on the live one, and it was already implemented and tested here — `test_fallback_orders_most_played_first`, `test_fallback_case_folding_still_matches_alongside_normalisation`. What was missing was the reference: `spec_lint.py`'s coverage regex could not see a lettered id, so FR-004a sat outside the check while it reported full coverage
...
- [x] T338 [US3] Wire the per-participant `replay` object into the match-detail response in `apps/api/src/aoe2stats_api/routers/matches.py`, with `download_path` and `obtainable_until` **null** for the two unobtainable states, and remove T335's T338-reasoned marker. The nulls are the requirement: FR-025 forbids presenting an unobtainable download as an action that then fails, and a null path is what makes rendering one impossible rather than discouraged
...
- [x] T343 [US3] Walk scenario 6, points 2 and 4, of `specs/003-player-search-match-analysis/quickstart.md` by hand — two points of view opened in AgeII:DE, and a match you played whose own point of view is archived while the others have expired — and record the outcome in the pull request. Confirm that obtaining any one of them takes at most two actions from the match page (SC-005). **Amended 2026-08-29**: this used to also ask the walker to confirm SC-004 — that the availability stated for every point of view matches what the source actually answers at that moment. Drop that confirmation: [research.md](./research.md) R8 records that SC-004 cannot be claimed while the split runs on `capture_budget_days` rather than the source's own measured window, and a match aged 22–31 days — past `CAPTURE_BUDGET_DAYS` (21) but still inside the source's own measured window on either contested reading — renders `expired` here by construction, not by a defect this walk could find
...
- [x] T372 [US4] Wire the analysis into the match page in `apps/web/src/features/analysis/`, including leaving and returning to a running analysis (FR-035) and offering the recompute only while the match-detail response reports `stale: true` (FR-041)
...
- [x] T378 Extend the free-tier watch in `scripts/checks/free_tier_watch.py` — **created by 001's T100, which lands first; if it has not, this task is blocked and does not create it**, because a second watch is the double-counting FR-048 forbids arriving through the monitor — to count the retained-recording prefix **separately** from 001's capture prefix, and to warn at 70 % of the analysis cap as well as of the bucket allowance. Counting them together would misstate both, which is the failure FR-048 exists to prevent, arriving through the monitor rather than through a query
