...
| --- | --- | --- |
| **I. Capture outranks analysis** | Analysis is the lowest-priority story, and the ordering is enforced in code rather than intended: an analysis fetch is refused while any capture sits inside its deadline danger window, analysis draws on its own smaller aoe.ms budget beneath capture's, and FR-047's storage cap is set below the allowance so capture keeps headroom it never has to compete for. `expired_total` staying at 0 is SC-008 and is asserted while the feature is in use. *Re-checked 2026-08-29 against I **4.1.0**, which removed the retention figure from the principle and added a ratchet: new evidence may tighten the capture budget, and may widen it only once `docs/data-sources.md` records the question as settled. Nothing in this feature widens any budget — analysis draws beneath capture and never from it — so the ratchet is satisfied by construction. What 4.1.0 does reach is R8's derivation model; see C2's decision record in [research.md](./research.md) R8.* | PASS |
...
| **IX. GDPR by design** | *Re-checked 2026-08-29 against IX **4.0.0**; this row previously read "IX at 3.0.1" and its whole argument was built on a version that no longer exists.* Two bases operate side by side and must not be conflated. **001's automatic capture** rests on legitimate interest (Art. 6-1-f) with a mandatory Art. 21 right to object — archival is on by default for a linked profile, and this feature adds nothing to it. **FR-033's retention** rests on IX's separate public-recording basis, which 4.0.0 left intact and which an archival objection does not reach. What made FR-033 lawful at 3.0.1 still does, and it attaches the same conditions: FR-045 registers the retention as its own processing purpose with its own legal basis, FR-047 caps and rate-limits it, and FR-046 — **amended 2026-08-24 with IX 3.0.0** — puts erasure and the third-party objection route on the *identifiers* held about a person appearing in a recording, pseudonymising them, while the recording and the analyses derived from it stand. IX orders principle IV above deletion here, deliberately; the processing register carries that ordering. FR-012 keeps automatic capture untouched — nothing is captured because a player was searched, viewed or favourited. That limit is now IX's *real* constraint rather than one clause among several: 4.0.0 retired the consent gate and left the point-of-view limit as what still forbids capturing a third party's recording. Note the boundary 4.0.0 draws and this row must not blur — a third party's public *match metadata* **is** recorded because somebody browsed (FR-011, by design); their *recording* is not (FR-012). FR-010 keeps third-party profiles out of any index. Regions unchanged. | PASS |
...
open, with `routers/privacy.py` implementing the archival objection and nothing else (**corrected 2026-08-29**: this read "implementing consent", true when written and false since T405 renamed the route to `POST /api/privacy/archival-objection`; no consent route exists). FR-033 creates a new category
...
3.0.0 (nor by 4.0.0) but its reason is: erasure no longer reaches the recording, so what T090 to T092 owe here is
