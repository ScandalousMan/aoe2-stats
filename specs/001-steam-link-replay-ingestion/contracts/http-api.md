...
| `GET`  | `/api/matches/{game_id}`                  | All participants, teams, civs, results, rating changes — each participant's `civ_id` is named as `civ_name` the same way (T070c), and the response itself carries `capture_status`/`capture_deadline_at` for this match, the identical fields the list row above carries (FR-027, T070e)                          |
