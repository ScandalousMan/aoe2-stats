...
`packages/providers`, `packages/storage`, `packages/design-system`, `infra/migrations`, and the one
platform-shaped file at `api/cron/ingest.py`.
...
- [ ] T001 Create the root `pyproject.toml` declaring the uv workspace members (`packages/core`, `packages/providers`, `packages/storage`, `apps/api`, `apps/ingester`), Python 3.13, and the shared `ruff` and `mypy` (strict) configuration, in `pyproject.toml`
- [ ] T002 [P] Create the five Python package skeletons with their own `pyproject.toml` and `src/` layout: `packages/core/`, `packages/providers/`, `packages/storage/`, `apps/api/`, `apps/ingester/`
...
- [ ] T004 [P] Configure pytest in `pyproject.toml` and add the autouse fixture in `tests/conftest.py` that makes the network unavailable when `PYTEST_DISABLE_NETWORK=1`, so constitution III is enforced by the harness and not by discipline
...
- [ ] T007 Implement every SQLAlchemy model from [data-model.md](./data-model.md) — `users`, `steam_identities`, `aoe_profiles`, `profile_links`, `matches`, `match_players`, `rating_snapshots`, `replay_captures`, `replay_parses`, `ingest_runs`, `provider_calls`, `replay_access_log`, `data_requests`, `sessions` — including the `(game_id, profile_id)` unique constraint that *is* deduplication and the partial unique index enforcing one primary profile per user, in `packages/storage/src/aoe2stats_storage/models.py`
...
- [ ] T013 [P] Implement the pluggable replay engine interface and the `aoe2rec-py` adapter used for capture-time validation only — well-formedness, inner filename, inner byte count, engine name and version — in `packages/core/src/aoe2stats_core/replay/validation.py`
...
- [ ] T018 Implement the ingester entrypoint skeleton `run_once(budget_seconds)` plus the time budget checked between items and never mid-item, and the ten-line cron handler that rejects a request without `CRON_SECRET` with 401, in `apps/ingester/src/aoe2stats_ingester/run.py`, `budget.py` and `api/cron/ingest.py`
...
- [ ] T030 [US1] Enforce the allowlist at account creation and return `not_allowlisted` with its explanation, in `apps/api/src/aoe2stats_api/routers/auth.py`
...
- [ ] T052 [US2] Implement the token bucket at one request per second to the replay endpoint, the backoff policy, and the rule that `ProviderRateLimited` stops the entire run and raises an alert rather than skipping one capture, in `apps/ingester/src/aoe2stats_ingester/ratelimit.py`
...
- [ ] T055 [US2] Implement capture: claim with `FOR UPDATE SKIP LOCKED` ordered by `capture_deadline_at` ascending, reclaim stale `downloading` rows older than the maximum function duration, download, validate through the engine interface (FR-026), upload the blob, verify the checksum, **then** update the row (FR-017, FR-023), in `apps/ingester/src/aoe2stats_ingester/capture.py`
- [ ] T056 [US2] Implement the `unavailable` versus `expired` decision in the caller from `matches.completed_at`, since the endpoint returns an identical 404 for both, and alert on `expired` only, in `apps/ingester/src/aoe2stats_ingester/capture.py`
...
- [ ] T058 [US2] Enforce the ingestion quota per user aggregated across all their profiles, never per profile (FR-044), in `apps/ingester/src/aoe2stats_ingester/budget.py`
...
- [ ] T060 [US2] Wire `POST /api/cron/ingest` to return 200 with the run report even when individual captures failed, reserving non-200 for a cycle that could not run at all, in `apps/api/src/aoe2stats_api/routers/health.py` and `api/cron/ingest.py`
- [ ] T061 [US2] Write the external checks and enable their jobs — the cron-liveness check failing past 30 hours and the capture audit asserting `expired_total == 0` with no capture pending beyond 7 days — in `scripts/checks/cron_liveness.py`, `scripts/checks/capture_audit.py` and `.github/workflows/nightly.yml`
...
- [ ] T073 [P] [US3] Write the component specs for the match row, the match detail panel and the capture-state badge — the badge must distinguish safe, still catchable with time remaining, and lost — in `packages/design-system/specs/`
...
- [ ] T097 [P] Add every personal data field introduced by this feature to the processing register, as constitution IX requires in the same change, in `docs/privacy/processing-register.md`
...
- [ ] T105 Walk `specs/001-steam-link-replay-ingestion/quickstart.md` by hand end to end, all eleven scenarios, against a real Steam account that has played online within the last week, recording the outcome of each in the pull request
