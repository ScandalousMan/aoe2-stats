...
2. **Find the one fact Relic does not serve.** Player colour comes from aoe2companion, whose
   response the provider already receives and discards; this is a parse widening on a wired,
   breaker-protected, degradable provider.
...
**Performance Goals**: No new per-request outbound call on any display path — the profile route stays
database-only, and the companion enrichment runs in the ingester and in the existing on-demand
third-party refresh, never in `GET /api/players/{id}`. Asset payload ≤10 MB total, WebP,
...
| -------- | --------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **I**    | Capture Outranks Analysis                     | **PASS**                                                | This is a display feature and it degrades no capture path. `upsert_match_player` widens to write columns from a response the same transaction already holds — no extra request, no change to `CAPTURE_BUDGET_DAYS`, no change to the capture enqueue. The backfill reads `raw_payload` from the database and never re-fetches. The one new outbound call (D2) is aoe2companion, which is not on the capture path and whose failure is degradation by contract. `expired_total` cannot move.                                                                                                                                                                                                |
...
| **III**  | All External Data Goes Through a DataProvider | **PASS**                                                | No new provider and no new endpoint — `MatchEnrichment` widens to carry per-participant fields already present in the response `enrich_matches` fetches. Timeout, retry, token bucket, circuit breaker and the `provider_calls` sink are inherited unchanged from `AsyncBaseProvider`. Asset packs are copied in at development time by a human; nothing fetches them at build, test or run time (feature 002's FR-011/FR-014 rule, adopted here). The Steam avatar is a client-side `<img src>`, not a connection from `apps/*` or `packages/core` — FR-015, and `tests/architecture/test_import_graph.py` keeps `httpx` out of both.                                                     |
...
| **C** | Companion per-participant enrichment → `color_id`                                                            | D2 — makes FR-003 possible at all                               |
...
| -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **FR-014 says this feature must not change what is ingested; wiring `enrich_matches` does.** | `color_id` is FR-003 and US1 acceptance scenario 5, and Relic serves no colour in any field — including `slotinfo` and `options`, both decoded and checked (research.md D2). aoe2companion is the only source, it is already a wired provider, and the fields sit inside a response the provider already receives and discards. | _Derive colour from team and ordering_ — an invention presented as a fact; AoE2:DE assigns colour by lobby slot, and feature 002's "gaps are decisions, not oversights" forbids interpolating. _Drop FR-003_ — a P1 scenario abandoned while its source sits parsed and thrown away. _Decode the blobs further_ — measured, they carry no colour, and GCUR forbids reverse-engineering the game to reach assets. The spec's FR-014 needs amending to permit enrichment through `packages/providers` while keeping its real constraint (no capture degradation, nothing outside providers), which constitution III and I both still enforce. |
