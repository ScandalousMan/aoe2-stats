...
- [x] T437 [US2] Widen `packages/design-system/src/components/ProfileSummary/` — alias as the heading, the numeric id as a secondary reference, the flag beside the name, the avatar leading — with stories for a full profile, an alias-less one and a country-less one. **The "No ratings yet" state is unchanged**: it is a real and correct outcome that profile `1807091` genuinely has, and every temptation in this task is to make it look like something went wrong. FR-007
...
- [x] T439 [US2] Run `visual-reviewer` locally over T436's and T437's stories, then `pnpm test:visual --changed`, updating baselines in `packages/design-system/__screenshots__/` where intended. Encodes the profile half of quickstart scenario 6. FR-013, SC-006
...
- [x] T449 [P] Derive a clean map **display name and pack slug from the raw `map_name`** so FR-002's "thumbnail and name derived from `map_name`" and FR-016's per-map thumbnail hold on production data, where `Match.map_name` is Relic's raw internal name (`Yucatan.rms`, `CoastalForest.rms`) rather than the clean fixture form. Today `packages/game-assets/src/index.ts`'s `mapThumbnail` lowercases and hyphenates the string as-is: `"Yucatan.rms"` → key `yucatan.rms`, which misses the pack, so a **standard** map (not a custom/tournament one) shows no thumbnail, and `apps/web/src/features/matches/mappers.ts` prints the raw `Yucatan.rms` as the label. This is the map analogue of the `civ_id → name` mapping feature 002 owns (FR-001, spec Assumption) — an internal-name → display-name(+slug) mapping, sourced the way feature 002's reference data was and recorded under the same discipline, **the placement and source of which is a plan decision to settle before implementing** (a lookup table vs a camelCase/`.rms`-strip heuristic; `CoastalForest` → `Coastal Forest` → `coastal-forest` does not fall out of a pure string transform). A name the mapping does not cover still degrades per FR-010 — a cleaned readable label with the `.rms` suffix stripped, no thumbnail, **no broken image**. This is `partial` against FR-002/FR-016/FR-010. Test-first in `packages/game-assets/src/index.test.ts` and `apps/web/src/features/matches/mappers.test.ts`: `"Yucatan.rms"` resolves to display `Yucatan` and a thumbnail URL under `/game-assets/maps/`; `"CoastalForest.rms"` resolves to `Coastal Forest` and its thumbnail; an uncovered `"SomeCustomMap.rms"` yields a cleaned label and `undefined` (no thumbnail, asserted on the return type as T408 does); the existing clean-name path (`"Arabia"`) still resolves unchanged (FR-002, FR-016, FR-010 — `partial`)
- [x] T450 [P] Run the read-time companion **colour enrichment on the profile match-history route** so FR-003's "each participant in the canonical player colour for their `color_id`" holds on `GET /api/players/{profile_id}/matches`, not only on `GET /api/matches`. `get_player_match_history` (`apps/api/src/aoe2stats_api/routers/players.py:474`) serves rows through `MatchesRepository.list_matches` + `match_row_json` but **never calls `_enrich_colours`** (`apps/api/src/aoe2stats_api/routers/matches.py`), so `color_id` stays `NULL` for a freshly-viewed profile and every swatch renders the neutral token — the identical-colour symptom the T447 walk saw on `1807091`. This route already reads the source live (`_refresh_third_party_history` on every call), so batched companion enrichment here crosses no boundary T419 protects — that assertion is about `GET /api/players/{profile_id}` (the profile summary), a different route. Reuse the existing `_enrich_colours`/`enrich_matches` path rather than a second copy (the search-storm/enrichment-storm sharing the router already documents), batched **at most once** over the page's `game_ids`, degraded companion writing **nothing and never `NULL`** (data-model §6). This is `partial` against FR-003. Test-first, extending `apps/api/tests/test_match_colour_enrichment.py` (or `test_players_history.py`) with the players-route mirror of the existing assertions: viewing a page makes at most one batched companion call; a returned colour is written to `match_players.color_id`; a degraded companion writes nothing and specifically not `NULL`; a second view makes no companion call (FR-003 — `partial`)

---

## Phase 8: Convergence

Appended by `/speckit-converge` after the Clarifications 2026-09-02 spec amendment declared the
four reversals the T447 walk motivated: on-view identity refresh (FR-017), ladder standing on view
(FR-018/SC-007), alias/avatar fetched on view (FR-007/FR-008a), and the country **Tooltip**
(FR-008). Each is a code-vs-spec gap now that the spec requires the behaviour. No new migration:
the `aoe_profiles` columns (`alias`, `country`, `avatar_hash`) and the `rating_snapshots` table all already exist. Constitution:
the on-view refresh reads public identity and standing only — never replay capture, never a
capture enqueue for a third party (FR-012 intact) — and its provider calls stay inside
`packages/providers` (constitution III), bounded by the per-user rate limits the matches/search
routes already apply. Test-first against the green-tree gate, same discipline as every phase above
(`xfail(strict=True, reason="<impl task> not implemented yet")`, module imported inside the test body,
the implementing half removing the marker; TypeScript tests written failing and turned green in the
same task).

- [x] T451 [P] Surface the Relic identity block so the on-view refresh has something to persist, per FR-007/FR-017 (missing). Relic's `getRecentMatchHistory` response carries a `profiles[]` array — `alias`, `country`, `profile_id` per player (`packages/providers/fixtures/relic/get_recent_match_history.json`) — that `RelicMatchHistoryProvider.recent_matches` (`packages/providers/src/aoe2stats_providers/relic/matches.py`) currently drops, extracting only `matchHistoryStats`. Widen it to also return that identity block (a `RawProfile`/`list[RawProfile]` of `profile_id → alias, country`, or an added field on the existing return), without changing the `RawMatch` shape it already yields. Test-first in `packages/providers/tests/`: the parsed result carries `profiles[].alias`/`country` keyed by `profile_id` from the fixture; a response with no `profiles[]` yields an empty identity set, never raises (FR-007, FR-017 — missing)
- [x] T452 Persist a real alias and country in `discover.touch_aoe_profile` (`apps/ingester/src/aoe2stats_ingester/discover.py:125`), per FR-007 (partial). Today it inserts `alias=str(profile_id)` as a placeholder and its `ON CONFLICT` sets only `last_seen_at`, so a real alias never lands. Accept optional `alias`/`country`; on insert use the real values when given; on conflict **overwrite the numeric-id placeholder with a real alias and set country**, but **never clobber an existing real alias with the `str(profile_id)` placeholder** — the exact inversion an implementer under pressure will get backwards, so assert both directions. Test-first in the ingester/storage tests: a first sight with a real alias stores it; a later sight with only the placeholder leaves the real alias intact; a first sight with no real alias stores the placeholder and a later real alias replaces it (FR-007 — partial)
- [x] T453 Build the shared on-view identity refresh and trigger it from **both** routes, per FR-017 (missing) and FR-008a (partial). A single helper in `apps/api/src/aoe2stats_api/routers/players.py` that, for a viewed `profile_id`: persists the alias/country from T451's identity block through T452's widened `touch_aoe_profile`; and fetches the avatar hash from the companion provider and persists it through `search.py`'s existing `persist_avatar_hashes` (the avatar hash rides companion's search endpoint keyed on a name, so this step resolves it via the alias the identity refresh just established — settle the exact companion lookup when implementing; degrade silently if companion is unavailable). Trigger the helper from `_refresh_third_party_history`'s callers on **both** `GET /api/players/{profile_id}` (the summary route) and `GET /api/players/{profile_id}/matches`. **This makes the summary route issue a provider call for the first time** — `test_players_routes.py`/`test_match_colour_enrichment.py`'s assertions that `GET /api/players/{profile_id}` makes *no* provider call (the T419/T426 property) are a **deliberate reversal** here and must be updated to the new contract, not worked around. Degrade discipline: any source failure falls back to stored data and never fails the view (the `_refresh_third_party_history` precedent). No capture enqueue, no `replay_captures` row (FR-012). Test-first: viewing a profile persists its real alias/country and, when companion answers, its avatar hash; a degraded source leaves the view answering from storage (FR-007, FR-008a, FR-017 — missing)
- [x] T454 [P] Fetch ladder standing on view and persist it, per FR-018/SC-007 (missing). `RelicProfileProvider.personal_stats(profile_ids)` (`packages/providers/src/aoe2stats_providers/relic/profile.py:114`) returns a `LeaderboardSnapshot` per leaderboard (rating, rank, wins, losses, streak, highest_rating) keyed by `profile_id` — the same call the ingester's `_refresh_ratings` (`discover.py`) uses for the signed-in user. Call it for the viewed `profile_id` as part of the on-view refresh (T453) and persist as `rating_snapshots`; the profile route's existing `_profile_ratings` already serves that shape, so the full card (rating, rank, record, streak, best) appears with no serialiser change. Keep the calm "No ratings yet" render only when the source returns no standing (SC-007's own clause). No new migration — `rating_snapshots` exists. Test-first in `apps/api/tests/`: viewing a profile with standing persists snapshots and the route serves the full card; a profile the source has no standing for still answers `200` with the empty-ratings state, never an error; a degraded source does not fail the view (FR-018, SC-007 — missing)
- [x] T455 [P] Author the country-Tooltip component spec, per FR-008 (missing). This is a `product-designer` invocation, not an implementer one (design-system skill point 1: implementers do not author `packages/design-system/specs/`). Write `packages/design-system/specs/tooltip.md` (the trigger/content/placement, the **keyboard-focus** and hover reveal, the dismiss behaviour, tokens only) and amend `country-flag`/`profile-summary` specs so the country name is the flag's tooltip content with an accessible name, no adjacent text element. Name the focus states explicitly — a later reviewer will assume they were covered (FR-008 — missing)
- [x] T456 Build the `Tooltip` component in `packages/design-system/src/components/` from T455's spec, tokens only, with `Tooltip.stories.tsx` and `Tooltip.test.tsx` in both themes, per FR-008 (missing). **Its reveal-on-focus and reveal-on-hover need an interaction test, not only a snapshot** — static story screenshots are blind to focus and hover (known from prior work on this repository), so a keyboard user's experience otherwise has no baseline. FR-013's tokens-only and story+visual-regression rules apply (FR-008 — missing)
- [x] T457 Wire the Tooltip into the country flag on view, per FR-008 (partial). In `packages/design-system/src/components/` and `apps/web/src/features/players/`, put the country name inside T456's `Tooltip` on the `CountryFlag`, and **remove the adjacent country text element** from `ProfileSummary` (the "France" text the T447 walk flagged). Extend the feature's existing tests and run `visual-reviewer` over the changed stories, then `pnpm test:visual --changed`, updating baselines only where intended (FR-008 — partial)
- [x] T458 Re-walk quickstart scenario 6 against production and run the full gate, closing the T447 loop, per SC-002/SC-007/FR-007/FR-008/FR-008a/FR-018. Run scenario 7's whole gate (as T446 did), then load `/players/1807091` and `/players/1807091/matches` signed-in and confirm the page now reads as a peer: alias **Azhague33** as the heading with the id demoted, country flag with the name in a hover/focus **tooltip** (no adjacent text), a real **avatar**, the **full ratings card** (not "No ratings yet"), map **thumbnails** and clean names (T449), and per-participant **colours** (T450). Record the outcome in the pull request — the walk is done when a reader cannot tell the page apart from a peer site on the SC-004 categories (SC-002, SC-007 — verification)
